#!/usr/bin/env python3
# Description: Tron Legacy Screensaver.
# Usage: python3 screensaver-tron-legacy.py
# Author: Justin Oros
...
try:
    pygame.mixer.music.load('screensaver-tron-legacy-music.mp3')
    pygame.mixer.music.play(-1)
except pygame.error as e:
    print(f"Error loading screensaver-tron-legacy-music.mp3: {e}")
    print("Continuing without music...")

try:
    wallpaper_original = pygame.image.load('screensaver-tron-legacy-wallpaper.jpg')
except pygame.error as e:
    print(f"Error loading screensaver-tron-legacy-wallpaper.png: {e}")
    pygame.quit()
    sys.exit()

try:
    disc_original = pygame.image.load('screensaver-tron-legacy-disc.png')
    DISC_SIZE = 180
    disc_original = pygame.transform.scale(disc_original, (DISC_SIZE, DISC_SIZE))
except pygame.error as e:
    print(f"Error loading screensaver-tron-legacy-disc.png: {e}")
    pygame.quit()
    sys.exit()
...
while running:
    ...
    for i in range(num_bars):
        ...
        for y in range(bar_height):
            alpha = int(255 * (y / bar_height))
            color = (125, 253, 254, alpha)
